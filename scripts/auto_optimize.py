...
def main():
    ...
    print("")
    print("=" * 60)
    print("AI Insight Hub 自动优化 - 第 {} 轮".format(round_num))
    print("=" * 60)
    print("")
    ...
    if round_num <= len(optimizations):
        ...
        log_action(task_name, "任务ID: {}".format(task_id))
        
        # 执行具体优化
        execute_optimization(task_id, round_num)
        
        # 提交更改
        commit_msg = "自动优化 #{}: {}".format(round_num, task_name)
        if git_commit(commit_msg):
            log_action("代码已提交", "Commit: {}".format(commit_msg))
        else:
            log_action("无需提交", "没有需要提交的更改")
    else:
        print("所有优化任务已完成！")
        return
    ...
    print("")
    print("=" * 60)
    print("第 {} 轮优化完成".format(round_num))
    print("下次优化: 8小时后")
    print("=" * 60)
    print("")
...
def add_dark_mode():
    ...
    if index_html.exists():
        ...
        # 添加深色模式切换按钮
        toggle_button = '''<button onclick="toggleTheme()" style="background: rgba(255,255,255,0.2); border: none; color: white; padding: 0.5rem 1rem; border-radius: 4px; cursor: pointer;">切换主题</button>'''
